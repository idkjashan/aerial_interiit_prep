...
from uav_sim_bringup.world_config import WORLDS, get_uav_pose


def find_model_sdf(model_name: str = 'x500_depth_down') -> Optional[str]:
    """Find the model SDF in PX4's model folder or on $GZ_SIM_RESOURCE_PATH."""
    px4_dir = os.environ.get('PX4_DIR', os.path.expanduser('~/PX4-Autopilot'))
    dirs = [os.path.join(px4_dir, 'Tools', 'simulation', 'gz', 'models')]
    dirs += [d for d in os.environ.get('GZ_SIM_RESOURCE_PATH', '').split(':') if d]
    for d in dirs:
        p = os.path.join(d, model_name, 'model.sdf')
        if os.path.isfile(p):
            return p
    return None

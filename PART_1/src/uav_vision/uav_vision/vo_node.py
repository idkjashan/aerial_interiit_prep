...
that translation, so this node stays independently testable.
  ...
  out: ~/odom          nav_msgs/Odometry           (ENU world, FLU body)
       ...
       ~/agl           std_msgs/Float32            height above ground from depth, m
...
class VoNode(Node):
    ...
    def _on_frame(self, rgb_msg, depth_msg):
        ...
        # Features are found in RGB pixels, but depth has its own resolution and FOV.
        def depth_lookup(pts):
            return depth_at_rgb_pixels(depth, pts, self.K_rgb, self.K_depth)

        t0 = self.get_clock().now().nanoseconds
        res = self.vo.step(rgb, depth, self.rpy, self.omega_flu, dt, agl, depth_lookup)

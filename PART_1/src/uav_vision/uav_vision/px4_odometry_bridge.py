...
Most of the details below come from PX4 v1.16 behaviour (checked in the source):

* ``pose_frame`` / ``velocity_frame`` have to be set explicitly. They default-construct to
...
* ``timestamp``/``timestamp_sample`` of 0 make PX4 substitute ``hrt_absolute_time()`` on
  arrival (see Tools/msg/templates/ucdr/msg.h.em). That is the simplest choice for
...
* A quaternion is rejected unless it is finite, non-zero and normalised to within 1e-5.
  To fuse position/velocity but not yaw, send NaN in q -- zeros are rejected.
* ``reset_counter`` should only increment on a real discontinuity (re-init / relocalisation).

#!/usr/bin/env bash
# Fast triage: is every link in the chain actually alive? Run while the sim is up.
echo "=== topic rates (5 s each; 0.0 means BROKEN) ==="
for t in /uav/rgb /uav/depth /uav/camera_info /fmu/out/vehicle_attitude \
         /fmu/out/sensor_combined /fmu/out/vehicle_local_position \
...
timeout 4 ros2 topic echo /fmu/out/vehicle_local_position --once 2>/dev/null \

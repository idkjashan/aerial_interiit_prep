...
class OffboardMission(Node):
    ...
    def _tick(self):
        ...
        if self.state == ARMING:
            self._heartbeat()
            self._setpoint_position([0.0, 0.0, 0.0])
            if self.offboard and not self.armed:
            ...
            if self.armed and self.offboard:
                ...
                self.state = TAKEOFF
                    ...
                    (now - self.mode_requested_at).nanoseconds * 1e-9 > 8.0:
                self.get_logger().error(
                    'OFFBOARD/arm not accepted after 8 s. Check: EV fusion running, '
        ...
        if self.state == TAKEOFF:
            ...
            if self.lpos is not None and abs(-self.lpos.z - self.alt) < self.arrival_r:
                ...
                self.get_logger().info(
                    f'reached {self.alt:.1f} m -> holding for {self.hold_s:.0f} s')
                self.state = HOLD
            return
